...
class Router:
    ...
    def recv_packet(self, packet, time_now):
        ...
        for ind in range(Router.routers):
            if self.hops[ind] == packet["source"].id:
                print(ind, table2[ind], self.second[ind][0])
    ...
    def recv_query(self, packet, time_now):
        if packet["source"] in [neighbour for neighbour in self.links.keys()]:
            ...
            pkt = Packet(self, packet["source"], self.vector, time_now)
            pkt.packet["query"] = True
    ...
    def draw(self, screen):
        ...
        if not self.active:
            ...
            pg.draw.circle(screen, (255, 255, 255), (self.x, self.y), self.radius, 10)
...
def init_nodes():
    ...
    link_nodes(nodes[0], nodes[3], 6)
    link_nodes(nodes[0], nodes[1], 2)
    link_nodes(nodes[1], nodes[2], 3)
    link_nodes(nodes[2], nodes[5], 6)
    link_nodes(nodes[2], nodes[4], 4)
    link_nodes(nodes[2], nodes[3], 4)
...
def receive_query(packet, time_now):
    dest = packet["dest"]
    dest.recv_query(packet, time_now)


def try_send_packet(source, dest, time_now):
    if source.vector[dest.id] == 49:
        if source.second[dest.id][0] == 49:
            source.send_query(time_now)
            source.req = dest
            print("No path found, activating {}".format(source.id))
        else:
            source.vector[dest.id] = source.second[dest.id][0]
            source.hops[dest.id] = source.second[dest.id][1]
            source.second[dest.id] = (49, "-")
            print("Sending Packet thru second hop")

    else:
        print("Sending Packet thru primary hop")
